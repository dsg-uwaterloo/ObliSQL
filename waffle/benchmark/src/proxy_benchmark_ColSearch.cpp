...
void fetchIndexedTable(bool stats,std::string tableName, std::string fetchColumn, std::string searchColumn, std::string searchValue, int client_batch_size, proxy_client client,std::atomic<int> &xput,std::vector<int> &latencies, metaData m) {
    ...
    trace_vector columnTrace;
    std::unordered_map<std::string, std::string> results;
    std::set<std::string> pkRange;
    while(pkstart!=pkEnd){
        std::string colKey = tableName+"/"+searchColumn+"/"+std::to_string(pkstart); 
        columnKeys.push_back(colKey);
     
        if(columnKeys.size() == client_batch_size){
            // std::cout<<get_keys[0]<<std::endl;
            // std::cout<<"Added to Trace Size:"<<get_keys.size()<<std::endl;
            columnTrace.push_back(std::make_pair(columnKeys, std::vector<std::string>()));
            columnKeys.clear();
        }
        pkstart++;
    }
    if(columnKeys.size()>0){
        // std::cout<<"Trace Left:"<<get_keys.size()<<std::endl;
        long dummy = client_batch_size - columnKeys.size();
        for(int i=0; i<dummy;i++){
            columnKeys.push_back(columnKeys[i]);
        }
        columnTrace.push_back(std::make_pair(columnKeys, std::vector<std::string>()));
    }
    std::cout<<"Colume Trace Size:"<<columnTrace.size()<<std::endl;
    int i=0;
    while (i!=columnTrace.size()) {
        auto keys_values_pair = columnTrace[i];
        if(keys_values_pair.second.empty()){
            auto res = client.get_batch(keys_values_pair.first); 
            for(int k=0; k<res.size();k++){
                std::string val = splitStringAndGetFirst(res[k],"|"); 
                if(val == searchValue){
                    std::string currKey = keys_values_pair.first[k]; 
                    std::istringstream iss(currKey);
                    std::string token;
                    std::string last;

                    // Split the string on "/"
                    while (std::getline(iss, token, '/')) {
                        last = token; // Store the current token
                    }
                    std::string fetchKey = tableName+"/"+fetchColumn+"/"+last; 
                    pkRange.insert(fetchKey);
                }
            }
        }
        ++i;
    ...
    std::vector<std::string> getKeys(pkRange.begin(),pkRange.end());
    getKeys.resize(client_batch_size,getKeys.front());
    auto result = client.get_batch(getKeys);
    if (stats) {
        rdtscll(end);
...
void client(int idx, int client_batch_size, int object_size, trace_vector &trace, std::string &output_directory, std::string &host, int proxy_port, metaData m,std::atomic<int> &xput) {
    ...
    fetchIndexedTable(true,m["tableName"].front(),"c_zip","c_state","B1",client_batch_size,client,xput,latencies,m);
...
int main(int argc, char *argv[]){
    ...
    date_string=date_string+"ColSearch";
