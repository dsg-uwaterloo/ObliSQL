...
std::vector<std::string> filterTable(std::unordered_map<std::string, std::string>table,std::string fetchColumn,std::string searchColumn,std::string searchValue,metaData m){
    std::string tableName = splitStringAndGetFirst(table.begin()->first,"/");
    std::vector<std::string> findPk; 
    std::vector<std::string> result;
    int pkstart = std::stoi(m["pkStart"].front());
    int pkEnd = std::stoi(m["pkEnd"].front());
    while (pkstart!=pkEnd) {
        std::string currKey = tableName+"/"+searchColumn+"/"+std::to_string(pkstart);
        std::string tableVal = table[currKey];
        std::string currValue = splitStringAndGetFirst(tableVal, "|");
        if(currValue==searchValue){
            findPk.push_back(std::to_string(pkstart));
        }
        ++pkstart;
    }
    for(std::string pk:findPk){
        std::string currKey = tableName+"/"+fetchColumn+"/"+pk;
        std::string val = table[currKey];
        result.push_back(val);
    }
    std::cout<<"Filtered Result Size: "<<result.size()<<std::endl;
    for(int i=0;i<result.size();i++){
        std::cout<<"PK: "<<findPk[i]<<"Value: "<<result[i]<<std::endl;
    }
    return result;
}

void fetchFullTable(bool stats,std::vector<int> &latencies, int client_batch_size, trace_vector &trace, std::atomic<int> &xput, proxy_client client, metaData m){
    ...
    while(i!=trace.size()){
        ...
        if(keys_values_pair.second.empty()){
            ...
            for(int k=0; k<res.size();k++){
                results[keys_values_pair.first[k]]=res[k];
            }
        }
        ++i;
    }
    filteredData = filterTable(results,"c_zip","c_state","B1",m); //Select c_zip from customer where c_state = "B1";
    ...
    std::cout<<"Batches Sent: "<<i<<"Trace Size: "<<trace.size()<<std::endl;
}
...
void run_benchmark(int run_time, bool stats, std::vector<int> &latencies, int client_batch_size,
    ...
    std::cout<<"Batches Sent: "<<i<<std::endl;
}
...
void client(int idx, int client_batch_size, int object_size, trace_vector &trace, std::string &output_directory, std::string &host, int proxy_port, metaData m,std::atomic<int> &xput) {
    ...
    fetchFullTable(true,latencies,client_batch_size,trace,xput,client,m);
...
int main(int argc, char *argv[]){
    ...
    date_string=date_string+"fullScan";
