...
func TestExecutor(t *testing.T) {
	...
	if err != nil {
		log.Fatal().Msgf("Couldn't Connect to Executor Proxy at localhost:9090. Error: %s", err)
	}
	newClient := executor.NewPlainTextExecutorClient(conn)
	//Testing Init DB
	initSuccess, err := newClient.InitDb(ctx, generateDataSet())
	if err != nil {
		log.Fatal().Msgf("Failed to Init DB! Error: %v", err)
	}

	if !initSuccess.Value {
		log.Fatal().Msgf("Init DB returned false! Failed to init DB")
	}
